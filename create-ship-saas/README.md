...
Scaffold a new ShipSaaS application with Next.js, Better Auth, and AI SDK.
...
1. **Update package.json**: Set your author, repository URL, and version in `create-ship-saas/package.json`
2. **Test locally** (optional): Test the package before publishing:
   ```bash
   cd create-ship-saas
   npm link
   cd /path/to/test/directory
   create-ship-saas my-test-app
   ```
...
Edit any files in the main project (outside of `create-ship-saas/` folder):
...
#### Step 2: Navigate to create-ship-saas Directory
...
**What this does:**
- Deletes everything in `create-ship-saas/template/`
- Copies ALL files from your main project into `template/`
- Excludes: `node_modules`, `.next`, `.git`, lock files, `.env`, and the `create-ship-saas` folder itself
- Cleans up the template's `package.json` (removes `private` field and `sync-template` script)

#### Step 4: Bump the Version

Update the version number in `create-ship-saas/package.json`:
...
**Option B - Manual:**
Edit `create-ship-saas/package.json` and change the version number.
...
```bash
cd /path/to/test-directory
npx create-ship-saas@latest test-project
# or use the specific version
npx create-ship-saas@1.1.4 test-project
```
...
- Users may have cached versions. Tell them to:
  ```bash
  npx clear-npx-cache
  npx create-ship-saas@latest my-project
  # or use specific version
  npx create-ship-saas@1.1.4 my-project
  ```
